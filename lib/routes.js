...
exports.setRoutes = function(app) {
    ...
    app.get('/', function(req, res) {
        views.homePage(function(homepage_context) {
            homepage_context.searchQuery = '';
            homepage_context.searchResults = {};
            homepage_context.searchResults.hits = [];
...
console.log(req.session);                        
    ...
    app.get('/search', function(req, res) {
        var q = req.query.q;
        var url = 'http://localhost:9200/projects/projects/_search?q=description:'+q;
        var options = {
            host: 'localhost',
            port: 9200,
        ...
        http.get(options, function(esResp) {
            esResp.on('data', function(esRespData) {
                var h = JSON.parse(esRespData.toString('utf8'));
                //res.end(JSON.stringify(h.hits));
                
                views.homePage(function(homepage_context) {
                    homepage_context.searchQuery = q;
                    homepage_context.searchResults = h.hits;
                    prepareContext(req, res, app, homepage_context, function(context) {
                        console.log(homepage_context);
                        res.render('layout.ejs', context);
                    });
                });
            });
 ...
 */  
var middleForm = function() {
    return function middleForm(req, res, next) {
        ...
        if (req.form) {
            req.form.complete(function(err, fields, files) {
                ...
                req.context.form = req.context.form || {};
                req.context.form.fields = fields;
                req.context.form.files = files;
