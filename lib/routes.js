...
//console.log(context);
    ...
    app.get('/auth/facebook', function(req, res){
        ...
        var redirect_uri = 'http://localhost:8080/auth/facebook/callback';
...
//console.log(oauth2);
            ...
            oauth2.getAccessToken(req.query.code,'http://localhost:8080/auth/facebook/callback', function(error, result) {
                console.log(result);
                if(error) {
                    console.error(error);
                } else {
                    // Do stuff with the access token (we are authenticated at this point)
                    res.send("Success.  Access token = "+result);
                }
            });  
        }
    });
    ...
    app.get('/fblogin', function(req, res) {
        // some sort of messge
        if(req.query.code) {
            var accessTokenUrl = 'https://graph.facebook.com/oauth/access_token?client_id='+auth.fb.appId+'&redirect_uri=http://localhost:8080/fblogin/&client_secret='+auth.fb.secret+'&code='+req.query.code;
            console.log(accessTokenUrl);
            https.get({ 
                host: 'graph.facebook.com', 
                path: '/oauth/access_token?client_id='+auth.fb.appId+'&redirect_uri=http://localhost:8080/fblogin/&client_secret='+auth.fb.secret+'&code='+req.query.code+'&grant_type=authorization_code' 
            }, 
            function(res) {
