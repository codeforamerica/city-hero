...
exports.setRoutes = function(app) {
    ...
    var authenticatedPageCntlr = joinMiddleware(
        tempControllers.users.hasAccess(['authenticated'])
    );
    
    var anonymousPageCntlr = joinMiddleware(
        tempControllers.users.notLoggedIn()
    );
    ...
    // User pages
    app.get('/profile/:uid', defaultPageContext(), context.users.isOwn(), context.users.profile(), function(req, res) {
        res.render('layout.ejs', res.context);
    });
    app.get('/profile/edit/:uid', defaultPageContext(), context.users.isOwn(), 
        context.users.profile(), context.users.ownerOnly(), function(req, res) {
        res.render('layout.ejs', res.context);
    });
    ...
    var middleRegister = joinMiddleware(
        exports.middleForm(),
        tempControllers.users.validateRegistration(),
        defaultPageContext(),
        anonymousPageCntlr(),
