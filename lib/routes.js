...
exports.setRoutes = function(app) {
     ...
     * Order matters.
     */
    var defaultPage = joinMiddleware(
    ...
    var formPage = joinMiddleware(
        defaultPage(), 
        exports.middleForm()
    );
    
    var authenticatedPage = joinMiddleware(
        controllers.users.hasAccess(['authenticated'])
    );
    
    var anonymousPage = joinMiddleware(
        controllers.users.notLoggedIn()
    );
    
    // Home page
    app.get('/', defaultPage(), pagesContext.home(), function(req, res) {
        res.render('layout.ejs', res.context);
    });

    // Project pages
    app.get('/projects/wizard', formPage(), projectsContext.wizard(), authenticatedPage(), function(req, res) {
        res.render('layout.ejs', res.context);
    });
    app.post('/projects/wizard', authenticatedPage(), function(req, res) {
    ...
    app.get('/templates/:tid/wizard', authenticatedPage(), function(req, res) {
        res.render('layout.ejs', res.context);
    });
    
    app.get('/projects/:pid', defaultPage(), projectsContext.details(), function(req, res) {
        res.render('layout.ejs', res.context);
    });
    app.get('/projects/edit/:pid', defaultPage(), projectsContext.edit(), 
        authenticatedPage(), controllers.projects.editOwn(), function(req, res) {
        res.render('layout.ejs', res.context);
    });
    ...
    // User pages
    app.get('/profile/:uid', defaultPage(), controllers.users.isOwn(), usersContext.profile(), function(req, res) {
        res.render('layout.ejs', res.context);
    });
    app.get('/user/register', defaultPage(), usersContext.register(), anonymousPage(), function(req, res) {
        res.render('layout.ejs', res.context);
    });
    ...
    app.get('/user/login', defaultPage(), usersContext.login(), anonymousPage(), function(req, res) {
        res.render('layout.ejs', res.context);
    });
     ...
     */
    app.get('/search', defaultPage(), pagesContext.home(), function(req, res) {
        var q = req.query.q;
