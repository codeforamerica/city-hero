...
exports.logout = function() {
    return function (req, res, next) {
        exports.setMessage(req, res, 'You have been successfully logged out.');
        req.session.fb = null;
...
exports.middleSessionContext = function() {
    return function (req, res, next) {        
        ...
        if (req.session.user && req.session.user.username) {
            ...
            res.context.session.nextUrl = req.session.nextUrl || '/';
        }
    ...
    if (req.form) {
            ...
            models.users.all(function(err, users) {
                ...
                for (var u in users) {
                    ...
                    if (current.username == submittedUsername && current.password == submittedPassword) {
                        ...
                        exports.setMessage(req, res, 'Successfully logged in.  Welcome back.');
                    }
                }
                
                if (!req.session.user || !req.session.user.username) {
                    exports.setMessage(req, res, 'User name or password not found.');
                }
        ...
        https.get({ host: 'graph.facebook.com', path: '/me?access_token='+accessToken }, function(fbRes) {
            ...
            fbRes.on('end', function() {
                ...
                // Get all, it would make more sense to create a view for this.
                models.users.all(function(err, users_data) {
                    // See if the user exists in our db
                    for (var u in users_data) {
                        var current = users_data[u].doc;
                        if (current.fbAccessToken && current.username == data.username) {
                            // User found
                            req.session.user = users_data[u].doc;
                            req.session.userLoggedIn = true;
                            exports.setMessage(req, res, 'Successfully logged in.  Welcome back.');
                        }
                    }
       ...
       /* ===> */       fbUserDoc.password = '12345'      /* <=== */
                        ...
                        users.createUser(fakeReq, res, function(err, context) {
                            res.redirect('/profile/' + context.username);
                        });
                    } else {
                        callback(err, users_data);
                    }
                });
            });
