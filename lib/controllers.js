...
exports.session = {
    ...
    getSessionContext: function() {
        return function (req, res, next) {
            ...
            req.context.session = req.context.session || {};

            // Check if user is logged in.
            if (req.session.user && req.session.user.username) {
                req.context.session.userLoggedin = true
                req.context.session.actionHref = '/logout';
                req.context.session.actionTitle = 'Log Out';
                req.context.session.user = req.session.user;
            }
            else {
                req.context.session.userLoggedin = false;
                req.context.session.actionHref = '/user/login';
                req.context.session.actionTitle = 'Login';
                req.context.session.user = {};
            }
