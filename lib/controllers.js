...
exports.session = {
    ...
    loginUser: function(req, res, callback) {
        if (req.form) {
            ...
            https.get({ host: 'graph.facebook.com', path: '/me?access_token='+accessToken }, function(res) {
                res.on('data', function(d) {
                    ...
                    models.Users.all(function(err, users) {
                        ...
                        if (!req.session.user || !req.session.user.username) {
                            ...
                            var x = JSON.parse(d.toString('utf8'));
                            x.fbAccessToken = accessToken;

                            exports.users.createUser(x, res, function(err, context) {
                                console.log(context);
...
exports.users = {
    ...
    createUser: function(req, res, callback) {
        if (req.form) {
            ...
            // This user wasn't in our db yet, so we need to add them
            console.log('in createUser');
            console.log(req);
            models.Users.save(req, function(err, resp) {
                callback(err, resp);
            });
        }
    }
