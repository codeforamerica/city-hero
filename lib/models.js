...
//console.log(docs);
    ...
    save: function(project, callback) {
        ...
        this.db.view('projectviews/byslug', function(err, projects) {
            ...
            var trialSlug = project_data.slug;
            
            // This code attempt to create a unique slug
            do {
                numLoops++;
                projects.forEach(function (proj) {
                    if(proj.slug === trialSlug) { foundMatch = true; }
                });
                if(!foundMatch) {
                    slugUnique = true;
...
exports.Users = {
    ...
    merge: function(user, callback) {
        // Get user
        this.get(user._id, function(err, existing) {
            var mergedUser = utils.combine(user, existing);
            this.save(mergedUser, function(err, response) {
